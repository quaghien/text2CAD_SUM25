...
def process_dataset(model, tokenizer, dataset, split_name, index_start=0, index_end=None, do_sample=True):
    
    # Select data range based on index_start and index_end
    if index_end is None:
        index_end = len(dataset)
    else:
        index_end = min(index_end, len(dataset))
    
    # Select the specified range from dataset
    filtered_dataset = dataset.select(range(index_start, index_end))
    
    predictions = []
    reasonings = []
    
    for item in tqdm(filtered_dataset, desc=f"Processing {split_name}"):
    ...
    processed_dataset = Dataset.from_dict({
        "uid": filtered_dataset["uid"],
        "input": filtered_dataset["input"],
        "reasoning": reasonings,
        "predicted_output": predictions,
        "ground_truth_output": filtered_dataset["output"]
    })

    # Add index suffix to split name
    split_name_with_index = f"{split_name}_{index_start}_{index_end}"
    processed_dataset.push_to_hub("wanhin/test_reasoning_1_2e", split=split_name_with_index)
...
def main():
    ...
    # Process English dataset without sampling - với index_start và index_end
    process_dataset(model, tokenizer, test_en, "no_sampling_en", index_start=3500, index_end=None, do_sample=False)

    # Process English dataset with sampling
    # process_dataset(model, tokenizer, test_en, "sampling_en", index_start=0, index_end=10, do_sample=True)

    # # Process Vietnamese dataset without sampling
    # process_dataset(model, tokenizer, test_vi, "no_sampling_vi", index_start=3500, index_end=None, do_sample=False)

    # # Process Vietnamese dataset with sampling
    # process_dataset(model, tokenizer, test_vi, "sampling_vi", index_start=0, index_end=10, do_sample=True)
